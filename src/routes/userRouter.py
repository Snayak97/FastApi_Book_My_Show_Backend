...
from src.schemas.userSchema import CreateUserModel,UserModel,UserLoginModel
from src.services.userServices import user_service
from src.auth.hashingPassword import Hash

from src.auth.tokens import Token
from datetime import timedelta, datetime
from src.config import Config

from src.db.redis import add_jwt_id_to_blocklist
from src.auth.dependency import RefreshTokenBearer,AccessTokenBearer,get_current_user,RollChecker

role_admin = Depends( RollChecker(["admin"]))
...
@user_router.patch("/{user_id}", dependencies=[role_admin])
async def make_admin(user_id: str, db: Session = Depends(get_db),token_details=Depends(access_token_bearer)):
    return await user_service.make_admin(user_id,db)
